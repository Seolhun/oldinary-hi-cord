...
@Controller
@Transactional(propagation = Propagation.REQUIRED, transactionManager = "txManager", noRollbackFor = {
		NullPointerException.class })
@RequestMapping("/board/{tableName}")
public class BoardController {
	...
	public String boardList(Model model, HttpServletRequest request, @PathVariable("tableName") String tableName) {
		...
		Paging paging = new Paging(currentPage, sType, rawQuestion, sDate, limit, tableName, null);
	...
	@Secured("SUPERADMIN")
	@RequestMapping(value = "/detail/{path}", method = RequestMethod.GET)
	public String boardDetail(Model model, @PathVariable("path") Long path,
			@PathVariable("tableName") String tableName, HttpServletRequest request, HttpServletResponse response) {
	...
	public String boardAddIn(Board board, @PathVariable("tableName") String tableName,
	...
	@RequestMapping(value = "/{operation}/{path}", method = RequestMethod.GET)
	public String boardProcess(Model model, @PathVariable("operation") String operation, @PathVariable("path") Long path,
		...
		if (operation.equals("delete")) {
			...
			if (boardService.update(temp) != null) {
				redirectAttributes.addFlashAttribute("deleteBoard", "success");
			} else {
				redirectAttributes.addFlashAttribute("deleteBoard", "unsuccess");
			}
		} else if (operation.equals("edit")) {
			Board editBoard = boardService.findById(path);
			if (editBoard != null) {
		...
		return "redirect:/board/" + tableName + "/list";
	}
