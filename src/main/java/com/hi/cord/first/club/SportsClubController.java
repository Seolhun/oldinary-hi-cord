...
public class SportsClubController {
	...
	public String inertClub(@Valid SportsClub sportsClub, ModelMap model, BindingResult result, HttpServletRequest request, Principal principal) throws Exception {
		...
		if (sportsClubName == null || sportsClubName.length() == 0) {
			...
			return mapping;
		} else if ((sportsClubAddress == null || sportsClubAddress.length() == 0)
				|| (sportsClubAddres2s == null || sportsClubAddres2s.length() == 0)) {
		...
		User user=userService.selectByEmail(principal.getName());
		sportsClub.setSportsClubOwner(user);
		sportsClubService.insert(sportsClub);
		return "redirect:/";
	}
